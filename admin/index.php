...
<body>
   ...
   <div class="p-4 sm:ml-64">
      <div class="p-4 border-2 border-gray-200 border rounded-lg dark:border-gray-700">
         ...
         <div class="flex grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4 justify-between">
         <a href="daftar_produk.php" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <?php
            // Koneksi ke database
            include('config.php');

            // Eksekusi query untuk menghitung jumlah produk
            $query = "SELECT COUNT(kd_obat) AS total_produk FROM detail_obat";
            ...
            if ($result) {
               ...
               echo "Query gagal dieksekusi: " . mysqli_error($mysqli);
               exit();
            }
            ...
            mysqli_close($mysqli);
            ?>
            <p class="font-normal text-gray-700">Total Produk</p>
            <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900"><?php echo $totalProduk ?></h5>
         </a>
         <a href="#" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            ...
            // Eksekusi query untuk menghitung jumlah produk
            $query = "SELECT COUNT(kd_kategori) AS kategori_produk FROM kategori_obat";
            $result = mysqli_query($mysqli, $query);

            // Periksa apakah query berhasil dieksekusi
            if ($result) {
               $row = mysqli_fetch_assoc($result);
               $totalKategori = $row['kategori_produk'];
            } else {
               echo "Query gagal dieksekusi: " . mysqli_error($mysqli);
               exit();
            }
            ...
            mysqli_close($mysqli);
            ?>
            <p class="font-normal text-gray-700">Total Kategori</p>
            <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900"><?php echo $totalKategori ?></h5>
         </a>
         <a href="pengguna.php" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <?php
            // Koneksi ke database
            include('config.php');

            // Eksekusi query untuk menghitung jumlah produk
            $query = "SELECT COUNT(id) AS total_pengguna FROM login";
            $result = mysqli_query($mysqli, $query);
            ...
            <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900"><?php echo $totalUser ?></h5>
         </a>
         </div>
      </div>
